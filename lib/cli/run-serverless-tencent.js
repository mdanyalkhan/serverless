...
module.exports = async () => {
  ...
  if (!(await isStandaloneInstalled())) {
    ...
    process.stdout.write(
      'Could not find "serverless-tencent" CLI in a system, starting installation...\n'
    );
    ...
    try {
      ...
      if (error.response && error.response.statusCode === 404) {
        process.stdout.write(
          `${[
            '',
            'Auto installation is not supported on this platform.',
            'Please install manually via "npm install -g serverless-tencent" and re-run the command',
          ].join('\n')}\n`
        );
        return;
      }
